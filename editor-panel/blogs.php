...
<div class="wrapper">
    ...
    <div class="content-wrapper bg-light" style="height: 600px; overflow-y: auto;">
        <div class="container container-fluid" style="padding-left: 15px">
            ...
            <div class="postheader bg-light" style="position: absolute; width: 80%; height: 190px; padding-top: 30px; padding-right: 50px; z-index: 5; border: 0;">
            ...
            <div class="bloglist" style="padding-top: 210px">
                <?php
                // Call the renderBlogs function to fetch the blog posts
                $user_id = $_SESSION['active_user_id'];
                $posts = renderBlogs($user_id);

                // Check if there are any posts
                if (empty($posts)) {
                    echo '<h6 class="text-center display-6">You haven\'t posted anything yet.</h6>';
                } else {
                    // Iterate over each post and render HTML dynamically
                    foreach ($posts as $post) {
                ?>
                <div class="blog-item row border rounded overflow-hidden shadow-sm h-md-250 position-relative col-md-11 bg-white" style="padding: 15px 10px 15px 15px; margin: 0 0 20px 2px">
                    <div class="col-md-7 d-flex flex-column">
                        <h3 class="display-6 title" style="font-family: 'Poppins'; font-weight: 750"><?php echo htmlspecialchars($post['title']); ?></h3>
                        <h5 class="text-muted poster" style="position: relative; bottom: 10px">By: <?php echo htmlspecialchars($post['fname'] . ' ' . $post['lname']); ?></h5>
                        <h6 class="text-muted ctgry" style="position: relative; bottom: 10px"><?php echo htmlspecialchars($post['category_name']); ?></h6>

                        <p style="width: 100%;" class="flex-grow-1">
                            <?php echo htmlspecialchars(shortenContent($post['content'], 300)); ?>
                        </p>
