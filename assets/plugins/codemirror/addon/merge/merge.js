...
(function(mod) {
  ...
  function syncScroll(dv, toOrig) {
    ...
    if (dv.mv.options.connect == "align") {
      ...
      var botassets/dist, mix;
      // Some careful tweaking to make sure no space is left out of view
      // when scrolling to top or bottom.
      if (targetPos > sInfo.top && (mix = sInfo.top / halfScreen) < 1) {
        targetPos = targetPos * mix + sInfo.top * (1 - mix);
      } else if ((botassets/dist = sInfo.height - sInfo.clientHeight - sInfo.top) < halfScreen) {
        var otherInfo = other.getScrollInfo();
        var botassets/distOther = otherInfo.height - otherInfo.clientHeight - targetPos;
        if (botassets/distOther > botassets/dist && (mix = botassets/dist / halfScreen) < 1)
          targetPos = targetPos * mix + (otherInfo.height - otherInfo.clientHeight - botassets/dist) * (1 - mix);
      }
    }
